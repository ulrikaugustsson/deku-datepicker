...
function createPicker(opts = {}) {
	...
	const DatePicker = {
		...
		render(component, setState) {
			const {pickerClass, callback, chooseText} = component.props;
			const {currentDate} = component.state;

			return (
				<div class={pickerClass || 'DatePicker'}>
					<YearPicker
						subtract={() => setState({currentDate: currentDate.subtract(1, 'year')})}
						add={() => setState({currentDate: currentDate.add(1, 'year')})}
						currentDate={currentDate} />

					<MonthPicker
						subtract={() => setState({currentDate: currentDate.subtract(1, 'month')})}
						add={() => setState({currentDate: currentDate.add(1, 'month')})}
						currentDate={currentDate} />

					<DayPicker
						setDay={dayNumber => setState({currentDate: currentDate.date(dayNumber)})}
						currentDate={currentDate} />

					<ChooseDayButton callback={() => callback(currentDate.toDate())} chooseText={chooseText || 'Choose Day'}/>
				</div>
			);
		}
	};
